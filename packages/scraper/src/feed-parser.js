...
export class RSSParser {
     ...
     */
    static parseXML(xmlContent, source) {
        const items = [];
        try {
            ...
            throw new ParseError(source, "Failed to parse RSS XML", error);
        }
        return items;
    }
    static parseRSSItem(itemXml, source) {
        try {
            ...
            const categories = categoryMatches?.map(match => match.replace(/<[^>]*>/g, "").trim()) || [];
            if (!title || !pubDate) {
                return null;
            }
            return {
...
export class ICalParser {
     ...
     */
    static parseICS(icsContent, source) {
        const items = [];
        try {
            ...
            throw new ParseError(source, "Failed to parse iCal ICS", error);
        }
        return items;
    }
    static extractEvents(icsContent) {
        const events = [];
    ...
    static parseICalEvent(eventText, source) {
        try {
    ...
    static parseICalProperties(eventText) {
        const properties = {};
    ...
    static unescapeICalValue(value) {
        return value
            .replace(/\\n/g, "\n")
            .replace(/\\,/g, ",")
            .replace(/\\;/g, ";")
    ...
    static parseICalDateTime(dtString, source) {
...
export class JSONFeedParser {
     ...
     * Parse JSON feed
     */
    static parseJSON(jsonContent, source) {
        try {
            ...
            if (error instanceof ParseError)
                throw error;
            throw new ParseError(source, "Failed to parse JSON feed", error);
        }
    }
    static parseJSONFeedFormat(feed, source) {
    ...
    static parseGenericEventArray(events, source) {
...
export class FeedParser {
     ...
     */
    static async fetchAndParse(url, source) {
        try {
            ...
            throw new NetworkError(source, url, error);
        }
    }
    /**
     * Parse content based on content type
     */
    static parseContent(content, contentType, source) {
